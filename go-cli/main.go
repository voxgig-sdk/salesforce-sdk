// Command Salesforce-cli is a CLI + REPL for the Salesforce SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SALESFORCE_APIKEY (api key), SALESFORCE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SALESFORCE_APIKEY carries the API key and
	// SALESFORCE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SALESFORCE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SALESFORCE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSalesforceSDK(opts)

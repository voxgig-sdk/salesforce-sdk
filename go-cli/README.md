...
boru-driven command-line client **and** interactive REPL for the Salesforce
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/salesforce-cli)
make build
...
./salesforce-cli list account
./salesforce-cli load 1 account            # {id:1} shorthand
./salesforce-cli load '{id:1}' account       # explicit match map
./salesforce-cli update '{name:"x"}' account

# 5. Override the API base URL for a single call
SALESFORCE_BASE=https://api.example.com ./salesforce-cli list account

# 6. No arguments -> interactive REPL
./salesforce-cli
salesforce> list account
salesforce> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/salesforce-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/salesforce-cli list account
   ```
...
```sh
./salesforce-cli load 1 account          # scalar shorthand for {id:1}
./salesforce-cli load '{id:1}' account     # explicit match map
```
...
```sh
./salesforce-cli update '{id:1,name:"new"}' account
```
...
Run with no arguments to open a REPL (prompt `salesforce>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/salesforce-cli`. |
...
(`./salesforce-cli <expr>`) and interactively (the REPL), and expressions compose the

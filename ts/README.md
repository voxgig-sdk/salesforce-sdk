# Salesforce TypeScript SDK



The TypeScript SDK for the Salesforce API — a type-safe, entity-oriented client with full async/await support.
...
```ts
const client = new SalesforceSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `SalesforceSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `SalesforceSDK.test(testopts?, sdkopts?)` | `SalesforceSDK` | Create a test-mode client. |
...
| `client` | `client(): SalesforceSDK` | Return the parent SDK client. |
...
salesforce/
├── src/
│   ├── SalesforceSDK.ts        # Main SDK class

...
const TEST_SEED = { "entity": { "account": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+SalesforceSDK\b/.test(code) ||
        /\bSalesforceSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+SalesforceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/SalesforceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);

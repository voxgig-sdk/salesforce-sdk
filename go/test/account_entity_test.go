...
func TestAccountEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		accountRef01MarkdefUp0Name := "CreatedDate"
...
func accountBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SALESFORCE_TEST_ACCOUNT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SALESFORCE_TEST_ACCOUNT_ENTID"])
	...
	if env["SALESFORCE_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewSalesforceSDK(core.ToMapAny(mergedOpts))
	}

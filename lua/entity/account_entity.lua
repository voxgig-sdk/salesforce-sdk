...
local AccountEntity = {}
AccountEntity.__index = AccountEntity


function AccountEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AccountEntity)
  self._name = "account"
  self._client = client
...
function AccountEntity:get_name()
  return self._name
end


function AccountEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AccountEntity.new(self._client, opts)
end
...
function AccountEntity:mark_deleted()
  self._deleted = true
end


function AccountEntity:deleted()
  return true == self._deleted
end


function AccountEntity:data_set(args)
  if args ~= nil then
...
function AccountEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AccountEntity:match_set(args)
  if args ~= nil then
...
function AccountEntity:match_get()
...
function AccountEntity:stream(action, args, callopts)
...
---@param reqmatch AccountLoadMatch
---@param ctrl? table
---@return Account
---@return string? err
function AccountEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AccountCreateData
---@param ctrl? table
---@return Account
---@return string? err
function AccountEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata AccountUpdateData
---@param ctrl? table
---@return Account
---@return string? err
function AccountEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch AccountRemoveMatch
---@param ctrl? table
---@return Account
---@return string? err
function AccountEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function AccountEntity:_run_op(ctx, post_done)
  local utility = self._utility

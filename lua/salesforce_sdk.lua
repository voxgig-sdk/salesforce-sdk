...
local SalesforceSDK = {}
SalesforceSDK.__index = SalesforceSDK
...
SalesforceSDK._make_feature = _make_feature


function SalesforceSDK.new(options)
  local self = setmetatable({}, SalesforceSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function SalesforceSDK:options_map()
...
function SalesforceSDK:get_utility()
  return Utility.copy(self._utility)
end


function SalesforceSDK:get_root_ctx()
  return self._rootctx
end


function SalesforceSDK:prepare(fetchargs)
...
function SalesforceSDK:direct(fetchargs)
...
-- by passing it.
function SalesforceSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function SalesforceSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "SalesforceSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function SalesforceSDK:Account(data)
...
function SalesforceSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = SalesforceSDK.new(sdkopts)
  sdk.mode = "test"

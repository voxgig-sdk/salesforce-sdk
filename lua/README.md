...
The Lua SDK for the Salesforce API — an entity-oriented client using Lua conventions.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/salesforce-sdk/releases)),
...
for _, item in ipairs(accounts) do
  print(item["id"], item["CreatedDate"])
end
```
...
```lua
-- Create
local created, err = client:Account():create({ AnnualRevenue = 1, CreatedDate = "example_CreatedDate" })
if err then error(err) end
...
```
lua/
├── salesforce_sdk.lua    -- Main SDK module
...
The main module (`salesforce_sdk`) exports the SDK constructor

...
[MCP](https://modelcontextprotocol.io) server exposing the Salesforce SDK as
two agent tools — `salesforce_list` and `salesforce_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/salesforce-mcp)
make build

# 2. Provide credentials via the environment
export SALESFORCE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user salesforce \
  -- /absolute/path/to/salesforce-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./salesforce-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// salesforce_list: first page of records
{ "entity": "account" }
{ "entity": "account", "query": { } }

// salesforce_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/salesforce-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user salesforce \
     -- "$PWD"/dist/*/salesforce-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `salesforce_list` and `salesforce_load` tools now appear
   in new sessions. Ask the agent to *"list account using salesforce"*
   and it calls `salesforce_list` with `{"entity":"account"}`.
...
```sh
export SALESFORCE_APIKEY=sk_live_xxx            # API key
export SALESFORCE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `salesforce_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `salesforce_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SALESFORCE_APIKEY` | API key sent with every request. |
| `SALESFORCE_BASE` | Optional override of the API base URL. |
...
```sh
./salesforce-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"salesforce_load","arguments":{"entity":"account","query":{"id":1}}}}'
```

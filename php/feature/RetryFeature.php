...
class SalesforceRetryFeature extends SalesforceBaseFeature
{
    ...
    public function init(SalesforceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SalesforceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SalesforceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SalesforceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

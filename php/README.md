...
The PHP SDK for the Salesforce API — an entity-oriented client using PHP conventions.
...
- Releases: [https://github.com/voxgig-sdk/salesforce-sdk/releases](https://github.com/voxgig-sdk/salesforce-sdk/releases)
...
try {
    ...
    foreach ($accounts as $record) {
        ...
        echo $item["id"] . " " . $item["AnnualRevenue"] . "\n";
    }
} catch (\Throwable $err) {
...
$created = $client->Account()->create(["AnnualRevenue" => 1, "CreatedDate" => "example_CreatedDate"]);

// Update — index the record via data_get() ($created->data_get()["id"]).
$client->Account()->update(["id" => $created->data_get()["id"], "AnnualRevenue" => 1, "CreatedDate" => "example_CreatedDate"]);
...
print_r(array_map(fn($item) => $item->data_get(), $account));
```
...
```php
require_once 'salesforce_sdk.php';
$client = new SalesforceSDK($options);
```
...
```php
$client = SalesforceSDK::test($testopts, $sdkopts);
```
...
```
php/
├── salesforce_sdk.php          -- Main SDK class
...
The main class (`salesforce_sdk.php`) exports the SDK class

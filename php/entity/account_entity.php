...
class AccountEntity
{
    ...
    {
        if ($args) {
            $this->_data = SalesforceHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = SalesforceHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *
     * @param AccountLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AccountLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Account|array The loaded Account as an assoc-array at the
     *   SDK boundary; throws SalesforceError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = SalesforceHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws SalesforceError on failure (item-5 convention).
     */
     ...
     *
     * @param AccountCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AccountCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Account|array The created Account as an assoc-array at the
     *   SDK boundary; throws SalesforceError on failure (item-5 convention).
     */
     ...
     *
     * @param AccountUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AccountUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Account|array The updated Account as an assoc-array at the
     *   SDK boundary; throws SalesforceError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an Account matching the given criteria.
     *
     * @param AccountRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; AccountRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Account|array The removed Account as an assoc-array at the
     *   SDK boundary; throws SalesforceError on failure (item-5 convention).
     */

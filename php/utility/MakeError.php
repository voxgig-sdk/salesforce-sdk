...
class SalesforceMakeError
{
    public static function call(?SalesforceContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new SalesforceContext([], null);
        }
        $op = $ctx->op ?? new SalesforceOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new SalesforceResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof SalesforceError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new SalesforceError('', $msg, $ctx);

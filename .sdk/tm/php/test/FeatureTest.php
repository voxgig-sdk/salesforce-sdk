...
class FtCtrl extends SalesforceControl
{
...
class FtClient
{
    ...
    private SalesforceUtility $utility;

    public function __construct(SalesforceUtility $utility, array $options)
    {
    ...
    public function get_utility(): SalesforceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SalesforceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new SalesforceUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (SalesforceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SalesforceConfig::make_feature($name);
    ...
    private static function buildUrl(SalesforceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SalesforceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SalesforceResponse($fetched);
        }
    ...
    private function fail(SalesforceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SalesforceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
class SalesforceTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SalesforceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SalesforceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SalesforceTestRunner.getenv("SALESFORCE_TEST_LIVE")
        override = SalesforceTestRunner.getenv("SALESFORCE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SalesforceTestRunner.getenv(key)
        ...
        explain = SalesforceTestRunner.getenv("SALESFORCE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SALESFORCE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if SalesforceTestRunner._test_control is not None:
            return SalesforceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SalesforceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SalesforceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SalesforceTestRunner.load_test_control()
...
def load_env_local():
    SalesforceTestRunner.load_env_local()


def env_override(m):
    return SalesforceTestRunner.env_override(m)


def entity_data(v):
    return SalesforceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SalesforceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SalesforceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SalesforceTestRunner.load_test_control()


def live_client_options():
    return SalesforceTestRunner.live_client_options()


def live_delay_ms():
    return SalesforceTestRunner.live_delay_ms()
